...
def get_pipeline():
    ...
    if _predict_pipeline is None:
        # ✅ Heavy imports happen lazily, only when first needed
...
def predict_datapoint():
    ...
    # ✅ CustomData also imported lazily
    from src.pipeline.predict_pipeline import CustomData

    data = CustomData(
        gender=request.form.get('gender'),
        race_ethnicity=request.form.get('ethnicity'),
        parent_education=request.form.get('parental_level_of_education'),
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", debug=True, use_reloader=False)  # ✅ No reloader overhead